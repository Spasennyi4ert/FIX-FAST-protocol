...
-import(fast_xml_utils, [get_attribute/2, get_attribute/3, get_bin_attribute/2, get_bin_attribute/3]).
...
init(RootElem, Options) ->
   ...
   Dicts1 = fast_dicts:new_dict(DictName, Dicts),
   Dicts2 = fast_dicts:new_dict(?type_dictionary(<<"any">>), Dicts1),
   {Dicts3, TList} = parse_template(RootElem, Dicts2, DictName, Options, fast_templates:init()),%% ?
   {Dicts3, #templates{
...
parse_template([XmlElem = #xmlElement{content = Childs} | Rest], Dicts, DefDict, Options, Templates) ->
   ...
   Dicts1 = fast_dicts:new_dict(DictName, Dicts),
   Dicts2 = fast_dicts:new_dict(?template_dictionary(TemplateName), Dicts1),
   ...
   Templates1 = fast_templates:add_template(Template, Templates),
...
parse_instruction([I = #xmlElement{name = typeRef} | Tail], Dicts, DefDict, Options) ->
   ...
   Dicts1 = fast_dicts:new_dict(?type_dictionary(TypeName), Dicts),
