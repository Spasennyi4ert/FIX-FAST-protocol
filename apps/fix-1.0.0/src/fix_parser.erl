...
decode_message([{msg_type,heartbeat}|Message]) -> % Heartbeat
  decode_fields(Message, #heartbeat{}, heartbeat, 5);

decode_message([{msg_type,test_request}|Message]) -> % TestRequest
  decode_fields(Message, #test_request{}, test_request, 5);

decode_message([{msg_type,resend_request}|Message]) -> % ResendRequest
  decode_fields(Message, #resend_request{}, resend_request, 6);

decode_message([{msg_type,reject}|Message]) -> % Reject
  decode_fields(Message, #reject{}, reject, 10);
...
decode_message([{msg_type,logout}|Message]) -> % Logout
  decode_fields(Message, #logout{}, logout, 6);

decode_message([{msg_type,execution_report}|Message]) -> % ExecutionReport
  decode_fields(Message, #execution_report{}, execution_report, 28);

decode_message([{msg_type,order_cancel_reject}|Message]) -> % OrderCancelReject
  decode_fields(Message, #order_cancel_reject{}, order_cancel_reject, 24);

decode_message([{msg_type,logon}|Message]) -> % Logon
  decode_fields(Message, #logon{}, logon, 13);

decode_message([{msg_type,new_order_single}|Message]) -> % NewOrderSingle
  decode_fields(Message, #new_order_single{}, new_order_single, 15);

decode_message([{msg_type,order_cancel_request}|Message]) -> % OrderCancelRequest
  decode_fields(Message, #order_cancel_request{}, order_cancel_request, 10);

decode_message([{msg_type,order_status_request}|Message]) -> % OrderStatusRequest
  decode_fields(Message, #order_status_request{}, order_status_request, 7);
...
field_index(heartbeat, msg_seq_num) -> 2;
field_index(heartbeat, sending_time) -> 3;
field_index(heartbeat, test_req_id) -> 4;
field_index(test_request, sender_comp_id) -> false;
field_index(test_request, target_comp_id) -> false;
field_index(test_request, msg_seq_num) -> 2;
field_index(test_request, sending_time) -> 3;
field_index(test_request, test_req_id) -> 4;
field_index(resend_request, sender_comp_id) -> false;
field_index(resend_request, target_comp_id) -> false;
field_index(resend_request, msg_seq_num) -> 2;
field_index(resend_request, sending_time) -> 3;
field_index(resend_request, begin_seq_no) -> 4;
field_index(resend_request, end_seq_no) -> 5;
field_index(reject, sender_comp_id) -> false;
field_index(reject, target_comp_id) -> false;
field_index(reject, msg_seq_num) -> 2;
field_index(reject, sending_time) -> 3;
field_index(reject, ref_seq_num) -> 4;
field_index(reject, ref_tag_id) -> 5;
field_index(reject, ref_msg_type) -> 6;
field_index(reject, session_reject_reason) -> 7;
field_index(reject, text) -> 8;
field_index(reject, encoded_text) -> 9;
field_index(sequence_reset, sender_comp_id) -> false;
field_index(sequence_reset, target_comp_id) -> false;
field_index(sequence_reset, msg_seq_num) -> 2;
field_index(sequence_reset, sending_time) -> 3;
field_index(sequence_reset, gap_fill_flag) -> 4;
field_index(sequence_reset, new_seq_no) -> 5;
field_index(logout, sender_comp_id) -> false;
field_index(logout, target_comp_id) -> false;
field_index(logout, msg_seq_num) -> 2;
field_index(logout, sending_time) -> 3;
field_index(logout, text) -> 4;
field_index(logout, encoded_text) -> 5;
field_index(execution_report, sender_comp_id) -> false;
field_index(execution_report, target_comp_id) -> false;
field_index(execution_report, msg_seq_num) -> 2;
field_index(execution_report, sending_time) -> 3;
field_index(execution_report, cl_ord_id) -> 4;
field_index(execution_report, orig_cl_ord_id) -> 5;
field_index(execution_report, exec_type) -> 6;
field_index(execution_report, ord_status) -> 7;
field_index(execution_report, side) -> 8;
field_index(execution_report, transact_time) -> 9;
field_index(execution_report, leaves_qty) -> 10;
field_index(execution_report, cum_qty) -> 11;
field_index(execution_report, last_px) -> 12;
field_index(execution_report, last_qty) -> 13;
field_index(execution_report, order_qty) -> 14;
field_index(execution_report, ord_type) -> 15;
field_index(execution_report, price) -> 16;
field_index(execution_report, ord_rej_reason) -> 17;
field_index(execution_report, text) -> 18;
field_index(execution_report, security_exchange) -> 19;
field_index(execution_report, market_segment_id) -> 20;
field_index(execution_report, symbol) -> 21;
field_index(execution_report, avg_px) -> 22;
field_index(order_cancel_reject, sender_comp_id) -> false;
field_index(order_cancel_reject, target_comp_id) -> false;
field_index(order_cancel_reject, msg_seq_num) -> 2;
field_index(order_cancel_reject, sending_time) -> 3;
field_index(order_cancel_reject, order_id) -> 4;
...
field_index(order_cancel_reject, orig_cl_ord_id) -> 9;
field_index(order_cancel_reject, ord_status) -> 10;
field_index(order_cancel_reject, working_indicator) -> 11;
field_index(order_cancel_reject, orig_ord_mod_time) -> 12;
field_index(order_cancel_reject, list_id) -> 13;
field_index(order_cancel_reject, account) -> 14;
field_index(order_cancel_reject, acct_id_source) -> 15;
field_index(order_cancel_reject, account_type) -> 16;
field_index(order_cancel_reject, trade_origination_date) -> 17;
field_index(order_cancel_reject, trade_date) -> 18;
field_index(order_cancel_reject, transact_time) -> 19;
field_index(order_cancel_reject, cxl_rej_response_to) -> 20;
field_index(order_cancel_reject, cxl_rej_reason) -> 21;
field_index(order_cancel_reject, text) -> 22;
field_index(order_cancel_reject, encoded_text) -> 23;
field_index(logon, sender_comp_id) -> false;
field_index(logon, target_comp_id) -> false;
field_index(logon, msg_seq_num) -> 2;
field_index(logon, sending_time) -> 3;
field_index(logon, encrypt_method) -> 4;
field_index(logon, heart_bt_int) -> 5;
field_index(logon, raw_data) -> 6;
field_index(logon, reset_seq_num_flag) -> 7;
field_index(logon, next_expected_msg_seq_num) -> 8;
field_index(logon, test_message_indicator) -> 9;
field_index(logon, username) -> 10;
field_index(logon, password) -> 11;
field_index(new_order_single, sender_comp_id) -> false;
field_index(new_order_single, target_comp_id) -> false;
field_index(new_order_single, msg_seq_num) -> 2;
field_index(new_order_single, sending_time) -> 3;
field_index(new_order_single, cl_ord_id) -> 4;
field_index(new_order_single, side) -> 5;
field_index(new_order_single, transact_time) -> 6;
field_index(new_order_single, order_qty) -> 7;
field_index(new_order_single, ord_type) -> 8;
field_index(new_order_single, price) -> 9;
field_index(new_order_single, stop_px) -> 10;
field_index(new_order_single, time_in_force) -> 11;
field_index(order_cancel_request, sender_comp_id) -> false;
field_index(order_cancel_request, target_comp_id) -> false;
field_index(order_cancel_request, msg_seq_num) -> 2;
field_index(order_cancel_request, sending_time) -> 3;
field_index(order_cancel_request, cl_ord_id) -> 4;
field_index(order_cancel_request, orig_cl_ord_id) -> 5;
field_index(order_cancel_request, side) -> 6;
field_index(order_cancel_request, transact_time) -> 7;
field_index(order_cancel_request, order_qty) -> 8;
field_index(order_status_request, sender_comp_id) -> false;
field_index(order_status_request, target_comp_id) -> false;
field_index(order_status_request, msg_seq_num) -> 2;
field_index(order_status_request, sending_time) -> 3;
field_index(order_status_request, cl_ord_id) -> 4;
field_index(order_status_request, side) -> 5;
...
field_by_number(<<"1300">>) -> market_segmeni_id;
...
decode_typed_field(market_segment_id, V) -> V;
...
%encode_typed_field(market_segment_id, 'futuresandstandart') -> <<"F">>;
encode_typed_field(market_segment_id, V) when is_integer(V) -> list_to_binary(integer_to_list(V));
...
number_by_field(market_segment_id) -> <<"1300">>;
