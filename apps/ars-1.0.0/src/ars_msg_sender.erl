...
handle_event({new_order_single, Pid, OrderId, Side, Quantaty, Options}, State) ->
    ?D(new_order_single),
    fix_exec_conn:new_order_single(Pid, OrderId, Side, Quantaty, Options),

    {ok, State};
handle_event({cancel_order, Pid, OrderId, CancOrdId, Side, Quantaty, Options}, State) ->
    fix_exec_conn:cancel_order(Pid, OrderId, CancOrdId, Side, Quantaty, Options),
    ?D(cancel_order),
    {ok, State};
