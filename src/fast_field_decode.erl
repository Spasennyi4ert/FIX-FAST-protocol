...
decode(Data, #templateRef{name = undef}, Context) ->
   {Msg, Data1, #context{dicts = D}} = fast_segment:decode(Data, Context),
...
decode(Data, #templateRef{name = Name}, Context = #context{template = T = #template{instructions = Instrs}}) ->
   TemplateRef = fast_templates:get_by_name(Name, Context#context.templates#templates.tlist),
...
decode(Data, #field{type = Type, disp_name = DispName, presence = Presence, operator = #copy{dictionary = D, key = Key}},
   ...
   case decode_type(Type, Data, is_nullable(Presence)) of
      {null, _, Data1} ->
         Dicts1 = fast_dicts:put_value(Dict, Key, empty, Dicts),
         {{DispName, absent}, Data1, Context#context{pmap = PMapRest, dicts = Dicts1}};
      {Value, Err, Data1} ->
         L(Err, Value),
         Dicts1 = fast_dicts:put_value(Dict, Key, Value, Dicts),
...
decode(Data, #field{name = FieldName, disp_name = DispName, presence = Presence, operator = #copy{dictionary = D, key = Key, value = InitialValue}},
   Context = #context{pmap = <<0:1, PMapRest/bits>>, dicts = Dicts,
      application = App, template = #template{name = TemplateName}}) ->
   Dict = select_dict(D, TemplateName, App),
   case fast_dicts:get_value(Dict, Key, Dicts) of
      ...
      undef ->
         Dicts1 = fast_dicts:put_value(Dict, Key, InitialValue, Dicts),
...
decode(Data, #field{type = Type, disp_name = DispName, presence = Presence, operator = #increment{dictionary = Dict, key = Key}},
   ...
   case decode_type(Type, Data, is_nullable(Presence)) of
      {Value, Err, Data1} ->
         L(Err, Value),
         Dicts1 = fast_dicts:put_value(select_dict(Dict, TemplateName, App), Key, Value, Dicts),
...
decode(Data, #field{type = Type, name = FieldName, disp_name = DispName, presence = Presence, operator = #increment{dictionary = D, key = Key, value = InitialValue}},
   ...
   case fast_dicts:get_value(Dict, Key, Dicts) of
      ...
      Value ->
         ...
         Dicts1 = fast_dicts:put_value(Dict, Key, NewValue, Dicts),
      ...
      {Delta, Err, Data1} ->
         ...
         case fast_dicts:get_value(Dict, Key, Dicts) of
            ...
            undef ->
               ...
               Dicts1 = fast_dicts:put_value(Dict, Key, NewVal, Dicts),
...
decode(Data,
   ...
   case NeedPMap of
      true ->
         fast_segment:decode_pmap(Data, Context);
      false ->
         {Data, Context}
   end,
   {Msg, Data2, #context{dicts = Dicts}} =
      fast_segment:decode_fields(Data1, Context1#context.template#template{instructions = Instrs}),
...
decode_sequence_aux(Length, Data, NeedPMap, Context = #context{template = Template}) ->
   case NeedPMap of
      true ->
         {Data1, Context1} = fast_segment:decode_pmap(Data, Context),
         {Msg, Data2, Context2} = fast_segment:decode_fields(Data1, Context1),
      ...
      false ->
         {Msg, Data1, Context1} = fast_segment:decode_fields(Data, Context),
...
create_context(PMap) ->
   Dicts = fast_dicts:init(),
   Dicts1 = fast_dicts:new_dict(global, Dicts),
   Dicts2 = fast_dicts:new_dict(?template_name, Dicts1),
