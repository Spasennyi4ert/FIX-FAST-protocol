...
% events
-export([ buy/2, sell/2, repeat_up/2, repeat_down/2, rejected/1, expired/1, 
...
orders_book({OrdStatus, _LQ, CQ, Side}) ->
    ...
    Prev_LQ = case ets:lookup(orders, rep) of
		   [{_, _, Prev_LQtys, _, _}] ->
		       Prev_LQtys;
		   _ ->
		       0
...
wait({buy, Price}, #state{pid_exec_conn=SendTo, pid_event=PidTo, count=Count} = State) ->
    ?D({buy_at, Price}),
    OrderQty = ?LIMIT,
    gen_event:notify(PidTo, {new_order_single, SendTo, Count, buy, OrderQty, [{ord_type,2},{price,Price}]}),
    {next_state, buying, State#state{count=Count+1, order_count=Count, order_qty=OrderQty, price=Price}};
wait({sell, Price}, #state{pid_exec_conn=SendTo, pid_event=PidTo, count=Count} = State) ->
    ?D({sell_at, Price}),
    OrderQty = ?LIMIT,
    gen_event:notify(PidTo, {new_order_single, SendTo, Count, sell, OrderQty, [{ord_type,2},{price, Price}]}),
    {next_state, selling, State#state{count=Count+1, order_count=Count, order_qty=OrderQty, price=Price}};
wait(_Event, State) ->
    {next_state, wait, State}.

buying(rejected, #state{} = State) ->
    ?D(rejected),
    {next_state, buy_cash, State};
buying(expired, #state{} = State) ->
    {next_state, buy_cash, State};
...
buying({repeat_up, Price},
       #state{pid_exec_conn=SendTo, pid_event=PidTo, count=Count, order_count=Order_Count, order_qty=OrderQty} = State) ->
    ?D({repeat_buy_at, Price}),
    gen_event:notify(PidTo, {cancel_order, SendTo, Count, Order_Count, buy, OrderQty, [{ord_type,2}]}),
    {next_state, canceled_buying, State#state{count = Count + 1, price = Price}};
buying({sell, Price},
       #state{pid_exec_conn = SendTo, pid_event = PidTo, count = Count, order_count = Order_Count, order_qty = OrderQty} = State) ->
    ?D({double_sell_at, Price, from_partial}),
    gen_event:notify(PidTo, {cancel_order, SendTo, Count, Order_Count, buy, OrderQty, [{ord_type,2}]}),
    {next_state, canceled_selling, State#state{count = Count + 1, price = Price}};
buying(ready_to_sell,
       #state{pid_exec_conn = SendTo, pid_event = PidTo, count = Count, order_count = Order_Count, order_qty = OrderQty} = State) ->
    ?D(ready_to_sell_from_partial),
    gen_event:notify(PidTo, {cancel_order, SendTo, Count, Order_Count, buy, OrderQty, [{ord_type,2}]}),
    {next_state, ready_sell, State#state{count = Count + 1}};
buying({market_sell, Price},
       #state{pid_exec_conn = SendTo, pid_event = PidTo, count = Count, order_count = Order_Count, order_qty = OrderQty} = State) ->
    ?D({market_sell_at, Price}),
    gen_event:notify(PidTo, {cancel_order, SendTo, Count, Order_Count, buy, OrderQty, [{ord_type,2}]}),
    {next_state, canceled_to_market_selling, State#state{count = Count + 1, price = Price}};
buying(_Event, State) ->
    {next_state, buying, State}.

selling(rejected, #state{} = State) ->
    ?D(rejected),
...
selling({repeat_down, Price},
    ...
    gen_event:notify(PidTo, {cancel_order, SendTo, Count, Order_Count, sell, OrderQty, [{ord_type,2}]}),
    {next_state, canceled_selling, State#state{count = Count + 1, price = Price}};
selling({buy, Price},
	#state{pid_exec_conn = SendTo, pid_event = PidTo, count = Count, order_count = Order_Count, order_qty = OrderQty} = State) ->
...
selling(ready_to_buy,
    ...
    gen_event:notify(PidTo, {cancel_order, SendTo, Count, Order_Count, sell, OrderQty, [{ord_type,2}]}),
    {next_state, ready_buy, State#state{count = Count + 1}};
selling({market_buy, Price},
       #state{pid_exec_conn = SendTo, pid_event = PidTo, count = Count, order_count = Order_Count, order_qty = OrderQty} = State) ->
    ?D({market_sell_at, Price}),
    gen_event:notify(PidTo, {cancel_order, SendTo, Count, Order_Count, sell, OrderQty, [{ord_type,2}]}),
    {next_state, canceled_to_market_buying, State#state{count = Count + 1, price = Price}};
selling(_Event, State) ->
    {next_state, selling, State}.

canceled_buying(canceled, #state{pid_exec_conn = SendTo, pid_event = PidTo, count = Count, price = Price} = State) ->
    ...
    if Pos < ?LIMIT ->
	    OrderQty = ?LIMIT - Pos,
	    gen_event:notify(PidTo, {new_order_single,SendTo,Count, buy, OrderQty, [{ord_type,2},{price, Price}]}),
	    {next_state, buying, State#state{count = Count + 1, order_count = Count, order_qty = OrderQty}};
       Pos >= ?LIMIT ->
...
canceled_selling(canceled, #state{pid_exec_conn = SendTo, pid_event = PidTo, count = Count, price = Price} = State) ->
    ...
    if Pos > -?LIMIT ->
	    OrderQty = ?LIMIT + Pos,
	    gen_event:notify(PidTo,{new_order_single, SendTo, Count, sell, OrderQty, [{ord_type,2},{price, Price}]}),
	    {next_state, selling, State#state{count = Count + 1, order_count = Count, order_qty = OrderQty}};
       Pos =< -?LIMIT ->
...
long({sell, Price}, #state{pid_exec_conn = SendTo, pid_event = PidTo, count = Count} = State) ->
    ?D({sell_at, Price}),
    OrderQty = 2*?LIMIT,
    gen_event:notify(PidTo, {new_order_single, SendTo, Count, sell, OrderQty, [{ord_type,2},{price, Price}]}),
    {next_state, selling, State#state{count = Count + 1, order_count = Count, order_qty = OrderQty, price = Price}};
...
long({market_sell, Price}, #state{pid_exec_conn = SendTo, pid_event = PidTo, count = Count} = State) ->
    ?D({sell_market_at, Price}),
    OrderQty = ?LIMIT,
    gen_event:notify(PidTo, {new_order_single, SendTo, Count, sell, OrderQty, [{ord_type,2},{price, Price}]}),
    {next_state, sell_cash, State#state{count = Count + 1, order_count = Count, order_qty = OrderQty, price = Price}};
long(_Event, State) ->
    {next_state, long, State}.

short({buy, Price}, #state{pid_exec_conn = SendTo, pid_event = PidTo, count = Count} = State) ->
    ?D({buy_at, Price}),
    OrderQty = 2*?LIMIT,
    gen_event:notify(PidTo, {new_order_single, SendTo, Count, buy, OrderQty, [{ord_type,2},{price, Price}]}),
    {next_state, buying, State#state{count = Count + 1, order_count = Count, order_qty = OrderQty, price = Price}};
...
short({market_buy, Price}, #state{pid_exec_conn = SendTo, pid_event = PidTo, count = Count} = State) ->
    ?D({buy_market_at, Price}),
    OrderQty = ?LIMIT,
    gen_event:notify(PidTo, {new_order_single, SendTo, Count, buy, OrderQty, [{ord_type,2},{price, Price}]}),
    {next_state, buy_cash, State#state{count = Count + 1, order_count = Count, order_qty = OrderQty, price = Price}};
short(_Event, State) ->
    {next_state, short, State}.

ready_sell({sell, Price}, #state{pid_exec_conn = SendTo, pid_event = PidTo, count = Count} = State) ->
    ?D({sell_at, Price}),
    Pos = case ets:lookup(orders, pos) of
	      [{_, Poss}] ->
		  Poss;
    ...
    if Pos > 0 ->
	    OrderQty = Pos,
	    gen_event:notify(PidTo,{new_order_single, SendTo, Count, sell, OrderQty,[{ord_type,2},{price, Price}]}),
	    {next_state, selling_cash, State#state{count = Count + 1, order_count = Count, order_qty = OrderQty, price = Price}};
       Pos == -?LIMIT ->
	    {next_state, short, State};
       Pos == 0 ->
	    {next_state, sell_cash, State};
       true ->
	    OrderQty = ?LIMIT + Pos,
	    {next_state, partial_short, State#state{order_qty = OrderQty}}
    end;
ready_sell(_Event, State) ->
    {next_state, ready_sell, State}.

ready_buy({buy, Price}, #state{pid_exec_conn = SendTo, pid_event = PidTo, count = Count} = State) ->
    ?D({buy_at, Price}),
    Pos = case ets:lookup(orders, pos) of
	      [{_, Poss}] ->
		  Poss;
    ...
    if Pos < 0 ->
	    OrderQty = -Pos,
	    gen_event:notify(PidTo,{new_order_single, SendTo, Count, buy, OrderQty,[{ord_type,2},{price, Price}]}),
	    {next_state, buying_cash, State#state{count = Count + 1, order_count = Count, order_qty = OrderQty, price = Price}};
       Pos == ?LIMIT ->
	    {next_state, long, State};
       Pos == 0 ->
	    {next_state, buy_cash, State};
       true ->
	    OrderQty = ?LIMIT - Pos,
	    {next_state, partial_long, State#state{order_qty = OrderQty}}
    end;
ready_buy(_Event, State) ->
    {next_state, ready_buy, State}.

partial_short({sell, Price}, #state{pid_exec_conn=SendTo, pid_event=PidTo, count=Count, order_qty = OrderQty} = State) ->
    gen_event:notify(PidTo,{new_order_single, SendTo, Count, sell, OrderQty, [{ord_type,2},{price, Price}]}),
    {next_state, selling, State#state{count = Count + 1, order_count = Count, price = Price}};
partial_short(_Event, State) ->
    {next_state, partial_short, State}.

partial_long({buy, Price}, #state{pid_exec_conn=SendTo, pid_event=PidTo, count=Count, order_qty = OrderQty} = State) ->
    gen_event:notify(PidTo,{new_order_single, SendTo, Count, buy, OrderQty, [{ord_type,2},{price, Price}]}),
    {next_state, buying, State#state{count = Count + 1, order_count = Count, price = Price}};
partial_long(_Event, State) ->
    {next_state, partial_long, State}.
...
buying_cash(_Event, State) ->
    {next_state, buying_cash, State}.

sell_cash({sell, Price}, #state{pid_exec_conn = SendTo, pid_event = PidTo, count = Count} = State) ->
    ?D({sell_from_cash_at, Price}),
    gen_event:notify(PidTo, {new_order_single, SendTo, Count, sell, ?LIMIT, [{ord_type,2},{price, Price}]}),
    {next_state, selling, State#state{count = Count + 1, order_count = Count, order_qty = ?LIMIT, price = Price}};
sell_cash(_Event, State) ->
    {next_state, sell_cash, State}.

buy_cash({buy, Price}, #state{pid_exec_conn = SendTo, pid_event = PidTo, count = Count} = State) ->
    ?D({buy_from_cash_at, Price}),
    gen_event:notify(PidTo, {new_order_single, SendTo, Count, buy, ?LIMIT, [{ord_type,2},{price, Price}]}),
    {next_state, buying, State#state{count = Count + 1, order_count = Count, order_qty = ?LIMIT, price = Price}};
...
canceled_to_market_buying(canceled, #state{pid_exec_conn = SendTo, pid_event = PidTo, count = Count, price = Price}) ->
    ...
    {next_state, buy_market,
     #state{pid_exec_conn = SendTo, pid_event = PidTo, count = Count + 1, order_count = Count, order_qty = OrderQty}};
       true ->
	    {next_state, buy_cash, #state{pid_exec_conn = SendTo, pid_event = PidTo, count = Count}}
    end;
canceled_to_market_buying(_Event, State) ->
    {next_state, canceled_to_market_buying, State}.
