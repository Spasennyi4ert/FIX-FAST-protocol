...
wait({buy, Price}, S = #state{pid_exec_conn=SendTo, pid_event=PidTo, count=Count}) ->
    ?D({buy_at, Price}),
    OrderQty = ?LIMIT,
...
wait({sell, Price}, S = #state{pid_exec_conn=SendTo, pid_event=PidTo, count=Count}) ->
    ?D({sell_at, Price}),
    OrderQty = ?LIMIT,
...
buying({partial, Qty}, S = #state{}) ->
    ?D({partial, Qty}),
    {next_state, buying, S#state{order_state = partial}};
...
selling({partial, Qty}, S = #state{}) ->
    ?D({partial, Qty}),
    {next_state, selling, S#state{order_state = partial}};
...
selling_cash({expired, Qty}, S = #state{}) ->
    {next_state, sell_cash, S#state{order_state = expired}};
...
buying_cash({expired, Qty}, S = #state{}) ->
    {next_state, buy_cash, S#state{order_state = expired}};
...
check_orders(S = #state{pid_exec_conn = SendTo, pid_event = PidTo, count = Count, order_count = Order_Count,
			order_qty = OrderQty, price = Price, side = Side, order_state = Order_State}) ->
    case Order_State of
	placed ->
	    gen_event:notify(PidTo, {cancel_order, SendTo, Count, Order_Count, Side, OrderQty, [{ord_type,2}]});
...
check_positions(S = #state{pid_exec_conn = SendTo, pid_event = PidTo, count = Count, order_count = Order_Count,
			order_qty = OrderQty, price = Price, side = Side, order_state = Order_State}) ->
    Pos = case ets:lookup(orders, pos) of
	      [{_, Poss}] ->
		  Poss;
